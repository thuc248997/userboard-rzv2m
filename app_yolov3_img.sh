...
#############################
echo -e "${YELLOW}>> Convert to PyTorch ${NC}"
cd ${WORK}/pytorch/${MODEL}
python3 ${WORK}/pytorch/${MODEL}/convert_to_pytorch.py
chmod +x ${WORK}/pytorch/${MODEL}/yolov3.pth
ls -l --color ${WORK}/pytorch/${MODEL}/yolov3.pth

#############################
echo -e "${YELLOW}>> Convert to ONNX ${NC}"
cd ${WORK}/pytorch/${MODEL}
...
/bin/cp -Rpfv rzv2m_drpai-sample-application_ver5.00/${APP_FOLDER}/exe/sample.bmp \
        meta-userboard-rzv2m/recipes-demo/${APP_NAME}/files
/bin/cp -Rpfv rzv2m_drpai-sample-application_ver5.00/${APP_FOLDER}/exe/coco-labels-2014_2017.txt \
        meta-userboard-rzv2m/recipes-demo/${APP_NAME}/files
exit 0
