...
Note please, according to the above u-boot settings, we don't need the tftp download, instead we use nfs download. The download method here is a little different from the method documented on Renesas Official guides. Thus we can bypass a step on the `tftp` settings. 
